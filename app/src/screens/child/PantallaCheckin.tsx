...
type AccionCheckin = 'nada' | 'calmarme' | 'entender' | 'hablar'
...
export default function PantallaCheckin({ navigation, route }: Props) {
  ...
  const [accionDeseada, setAccionDeseada] = useState<AccionCheckin | null>(null)
  ...
  function puedeContinuar() {
    ...
    if (paso === 4) return !!accionDeseada
    return false
  }
  ...
  async function guardarCheckin() {
    ...
    try {
      await insertarCheckin({
        ...
        wants_to_talk: accionDeseada === 'hablar',
        wants_only_help: accionDeseada === 'calmarme',
      })
      // Navegar según la acción elegida en el paso 5
      if (accionDeseada === 'calmarme') {
        navigation.replace('Calmarme', { perfil })
      } else if (accionDeseada === 'hablar') {
        navigation.replace('AyudameADecirlo', { perfil })
      } else {
  ...
  return (
    <SafeAreaView style={estilos.contenedor}>
      ...
      >
        ...
        {/* Paso 4: Que quieres hacer */}
        {paso === 4 && (
          <PasoAccion
            accion={accionDeseada}
            onSeleccionar={setAccionDeseada}
          />
        )}
      </ScrollView>
...
// ─── Paso 5: Acción deseada ─────────────────────────────────────

const OPCIONES_ACCION: { id: AccionCheckin; etiqueta: string; descripcion: string; color: string }[] = [
  { id: 'nada',     etiqueta: 'Ya me siento mejor',       descripcion: 'Registrarlo fue suficiente',      color: COLORES.verdeApagado },
  { id: 'calmarme', etiqueta: 'Calmarme',                 descripcion: 'Ejercicio de respiracion',         color: COLORES.azulsuave },
  { id: 'entender', etiqueta: 'Entender lo que siento',   descripcion: 'Pensar un poco en ello',           color: COLORES.arenaCalida },
  { id: 'hablar',   etiqueta: 'Hablar con alguien',       descripcion: 'Ayudame a decirlo',                color: COLORES.superficie },
]
...
function PasoAccion({
  ...
  accion: AccionCheckin | null
  onSeleccionar: (a: AccionCheckin) => void
}) {
  return (
    <View>
      <Text style={estilos.preguntaTitulo}>Que quieres hacer?</Text>
      <Text style={estilos.preguntaSubtitulo}>Elige una opcion</Text>
      ...
      <View style={estilos.listaOpciones}>
        {OPCIONES_ACCION.map(op => (
          <TouchableOpacity
            ...
            style={[
              estilos.filaOpcion,
              accion === op.id && { backgroundColor: op.color },
              accion === op.id && estilos.filaOpcionActiva,
            ]}
            onPress={() => onSeleccionar(op.id)}
            activeOpacity={0.8}
          >
            <View style={[
              estilos.circuloOpcion,
              accion === op.id && estilos.circuloOpcionActivo,
            ]} />
            <View>
              <Text style={[
                estilos.etiquetaOpcion,
                accion === op.id && estilos.etiquetaOpcionActiva,
              ]}>
                {op.etiqueta}
              </Text>
              <Text style={estilos.relacionTexto}>{op.descripcion}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
